...
<h2 class="font-bold text-4x1 text-blue-700">Categories</h2>
...
@foreach($categories as $category)
        <tr>
            ...
            <td>{{$category->name}}</td>
            <td>
                <a href="{{route('category.edit',$category->id)}}" class="bg-blue-500 text-white px-2 py-1 rounded hover:shadow-blue-400">Edit</a>
              
                {{--<a onclick=" return confirm('Are you sure to delete?')" href="{{route ('category.destroy',$category->id)}}"class="bg-red-600 text-white px-2 py-1 rounded hover:shadow_red-400">Delete</a> --}}
                <a onclick="showDelete({{$category->id}})" class="bg-red-600 text-white px-2 py-1 rounded hover:shadow_red-400">Delete</a>
